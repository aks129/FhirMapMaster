...
def generate_python_code(mappings, fhir_standard, df=None):
    """
    ...
    Args:
        ...
        df: Optional DataFrame for enhanced type mapping
    
    Returns:
        str containing the Python code
    """
    # Use enhanced mapper if DataFrame is provided
    if df is not None:
        try:
            from utils.enhanced_mapper import generate_enhanced_mapping_code
            return generate_enhanced_mapping_code(mappings, df, fhir_standard)
        except Exception as e:
            st.warning(f"Error using enhanced mapper: {str(e)}. Falling back to standard mapper.")
    code = f"""
...
def export_mapping_as_file(export_format, mappings, fhir_standard, df=None):
    ...
    if export_format == "python":
        content = generate_python_code(mappings, fhir_standard, df)
